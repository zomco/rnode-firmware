...
void setTXPower() {
	if (radio_online) {
		...
		if (model == MODEL_1C) LoRa->setTxPower(mapped_lora_txp, PA_OUTPUT_PA_BOOST_PIN);
		if (model == MODEL_1D) LoRa->setTxPower(mapped_lora_txp, PA_OUTPUT_PA_BOOST_PIN);
    ...
    if (model == MODEL_C7) LoRa->setTxPower(mapped_lora_txp, PA_OUTPUT_RFO_PIN);
		...
		if (model == MODEL_D7) LoRa->setTxPower(mapped_lora_txp, PA_OUTPUT_PA_BOOST_PIN);
		if (model == MODEL_D8) LoRa->setTxPower(mapped_lora_txp, PA_OUTPUT_PA_BOOST_PIN);
...
bool eeprom_model_valid() {
	...
	#elif BOARD_MODEL == BOARD_EORA_S3
	if (model == MODEL_D7 || model == MODEL_D8) {
	#elif BOARD_MODEL == BOARD_XIAO_S3
  ...
  #elif BOARD_MODEL == BOARD_XIAOWA
  if (model == MODEL_1C || model == MODEL_1D) {
	#elif BOARD_MODEL == BOARD_HUZZAH32
	if (model == MODEL_FF) {
	#elif BOARD_MODEL == BOARD_GENERIC_ESP32
	if (model == MODEL_FF || model == MODEL_FE) {
